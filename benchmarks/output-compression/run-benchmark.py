#!/usr/bin/env python3
"""Output compression benchmark: 10 tasks x 5 arms via Anthropic API.

Measures how output compression levels affect token usage and accuracy.
Requires ANTHROPIC_API_KEY environment variable. Estimated cost: ~$0.50/run
with Sonnet.

LOCAL-ONLY benchmark. Not run in CI.

Arms:
  1. verbose (level 0) -- no compression instructions
  2. standard (level 1) -- drop pleasantries and preamble
  3. terse (level 2) -- drop articles, filler, hedging
  4. minimal (level 3) -- structured data only
  5. caveman-full -- full caveman mode instructions

Usage:
    export ANTHROPIC_API_KEY=sk-ant-...
    python3 run-benchmark.py
    python3 run-benchmark.py --tasks explain-convergence,review-summary
    python3 run-benchmark.py --model claude-sonnet-4-20250514
    python3 run-benchmark.py --dry-run
"""

import argparse
import json
import os
import re
import sys
import time

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = os.path.dirname(os.path.dirname(SCRIPT_DIR))
TASKS_DIR = os.path.join(REPO_ROOT, "evals", "tasks")
RESULTS_JSON = os.path.join(SCRIPT_DIR, "results.json")
RESULTS_MD = os.path.join(SCRIPT_DIR, "results.md")

MULTIPLIER = 1.3

# System prompt prefixes for each arm
ARM_PROMPTS = {
    "verbose": "",
    "standard": (
        "OUTPUT COMPRESSION -- STANDARD MODE\n\n"
        "Drop: pleasantries (sure/certainly/I'd be happy to), restated context "
        "(as you mentioned/based on the requirement), transition phrases "
        "(moving on to/now let's look at).\n"
        "Keep: full sentences, technical detail, code blocks, error messages.\n\n"
    ),
    "terse": (
        "OUTPUT COMPRESSION -- TERSE MODE\n\n"
        "Drop: articles (a/an/the), filler (just/really/basically/simply), "
        "pleasantries (sure/certainly/I'd be happy to), hedging (perhaps/might/"
        "you could consider), restated context (as you mentioned/based on the "
        "requirement), transition phrases (moving on to/now let's look at).\n"
        "Keep: technical terms exact, code blocks unchanged, error messages "
        "verbatim, file paths, line numbers, finding categories, severity levels.\n"
        "Pattern: [subject] [action] [reason]. [next step].\n\n"
    ),
    "minimal": (
        "OUTPUT COMPRESSION -- MINIMAL MODE\n\n"
        "Output ONLY structured data. No prose sentences.\n"
        "Findings: pipe-delimited format.\n"
        "Test results: pass/fail counts, failing test names.\n"
        "Lint results: file:line: issue.\n"
        "Code: code blocks only, no explanation unless ambiguous.\n\n"
    ),
    "caveman-full": (
        "CAVEMAN MODE -- FULL COMPRESSION\n\n"
        "U talk like caveman. Short word only. No article. No filler.\n"
        "Drop: a/an/the, just/really/basically, sure/certainly, "
        "however/furthermore/additionally.\n"
        "Keep: code exact, technical term exact, number exact.\n"
        "Pattern: [thing] [do] [why]. No fluff.\n"
        "Example: 'Auth middleware skip expiry check. Fix: add isExpired() guard.'\n\n"
    ),
}

# Forge context (shared across all arms)
FORGE_CONTEXT = (
    "You are a forge pipeline agent. Forge is a 10-stage autonomous development "
    "pipeline with 41 agents, quality scoring (score = max(0, 100 - 20*CRITICAL - "
    "5*WARNING - 2*INFO)), convergence engine, and recovery system. "
    "PASS >= 80, CONCERNS 60-79, FAIL < 60."
)


def load_tasks(task_filter: list = None) -> list:
    """Load task definitions from evals/tasks/*.md."""
    tasks = []
    if not os.path.isdir(TASKS_DIR):
        print(f"ERROR: Tasks directory not found: {TASKS_DIR}", file=sys.stderr)
        sys.exit(1)

    for filename in sorted(os.listdir(TASKS_DIR)):
        if not filename.endswith(".md"):
            continue

        path = os.path.join(TASKS_DIR, filename)
        with open(path, "r", encoding="utf-8") as f:
            content = f.read()

        # Parse YAML frontmatter
        fm_match = re.match(r"^---\n(.*?)\n---", content, re.DOTALL)
        if not fm_match:
            continue

        fm_text = fm_match.group(1)
        task = {"file": filename}

        for line in fm_text.split("\n"):
            line = line.strip()
            if line.startswith("id:"):
                task["id"] = line.split(":", 1)[1].strip().strip('"')
            elif line.startswith("name:"):
                task["name"] = line.split(":", 1)[1].strip()
            elif line.startswith("prompt:"):
                task["prompt"] = line.split(":", 1)[1].strip().strip('"')
            elif line.startswith("  - "):
                task.setdefault("required_facts", []).append(
                    line.lstrip("  - ").strip().strip('"')
                )

        if task_filter:
            if task.get("name") not in task_filter and task.get("id") not in task_filter:
                continue

        if "prompt" in task and "required_facts" in task:
            tasks.append(task)

    return tasks


def count_words(text: str) -> int:
    return len(text.split())


def estimate_tokens(text: str) -> int:
    return round(count_words(text) * MULTIPLIER)


def check_accuracy(response: str, required_facts: list) -> dict:
    """Check substring matches for required facts."""
    response_lower = response.lower()
    matches = []
    misses = []
    for fact in required_facts:
        if fact.lower() in response_lower:
            matches.append(fact)
        else:
            misses.append(fact)
    return {
        "matches": matches,
        "misses": misses,
        "accuracy": len(matches) / len(required_facts) if required_facts else 0.0,
    }


def call_api(prompt: str, system: str, model: str) -> dict:
    """Call Anthropic API and return response with metadata."""
    try:
        import anthropic
    except ImportError:
        print(
            "ERROR: anthropic package required. Install with: pip install anthropic",
            file=sys.stderr,
        )
        sys.exit(1)

    client = anthropic.Anthropic()
    start = time.time()
    response = client.messages.create(
        model=model,
        max_tokens=2000,
        system=system,
        messages=[{"role": "user", "content": prompt}],
    )
    elapsed = time.time() - start

    text = response.content[0].text if response.content else ""
    return {
        "text": text,
        "input_tokens": response.usage.input_tokens,
        "output_tokens": response.usage.output_tokens,
        "estimated_output_tokens": estimate_tokens(text),
        "words": count_words(text),
        "elapsed_seconds": round(elapsed, 2),
        "model": response.model,
    }


def run_benchmark(tasks: list, model: str, dry_run: bool = False) -> dict:
    """Run all tasks across all arms."""
    results = {
        "meta": {
            "model": model,
            "multiplier": MULTIPLIER,
            "token_method": "word_count * 1.3 (estimated) + API usage (actual)",
            "arms": list(ARM_PROMPTS.keys()),
            "task_count": len(tasks),
            "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        },
        "tasks": {},
    }

    total_calls = len(tasks) * len(ARM_PROMPTS)
    call_num = 0

    for task in tasks:
        task_name = task["name"]
        task_results = {"prompt": task["prompt"], "required_facts": task["required_facts"], "arms": {}}

        for arm_name, arm_prefix in ARM_PROMPTS.items():
            call_num += 1
            system = arm_prefix + FORGE_CONTEXT
            prompt = task["prompt"]

            print(f"  [{call_num}/{total_calls}] {task_name} / {arm_name}", end="", flush=True)

            if dry_run:
                # Estimate without API call
                task_results["arms"][arm_name] = {
                    "text": "(dry-run)",
                    "input_tokens": estimate_tokens(system + prompt),
                    "output_tokens": 0,
                    "estimated_output_tokens": 0,
                    "words": 0,
                    "elapsed_seconds": 0,
                    "accuracy": 0.0,
                    "matches": [],
                    "misses": task["required_facts"],
                }
                print(" (dry-run)")
                continue

            try:
                api_result = call_api(prompt, system, model)
                accuracy = check_accuracy(api_result["text"], task["required_facts"])

                task_results["arms"][arm_name] = {
                    "text": api_result["text"],
                    "input_tokens": api_result["input_tokens"],
                    "output_tokens": api_result["output_tokens"],
                    "estimated_output_tokens": api_result["estimated_output_tokens"],
                    "words": api_result["words"],
                    "elapsed_seconds": api_result["elapsed_seconds"],
                    "accuracy": accuracy["accuracy"],
                    "matches": accuracy["matches"],
                    "misses": accuracy["misses"],
                }
                print(
                    f" -- {api_result['output_tokens']} tokens, "
                    f"accuracy {accuracy['accuracy']:.0%}, "
                    f"{api_result['elapsed_seconds']}s"
                )
            except Exception as e:
                print(f" ERROR: {e}")
                task_results["arms"][arm_name] = {
                    "text": f"ERROR: {e}",
                    "input_tokens": 0,
                    "output_tokens": 0,
                    "estimated_output_tokens": 0,
                    "words": 0,
                    "elapsed_seconds": 0,
                    "accuracy": 0.0,
                    "matches": [],
                    "misses": task["required_facts"],
                    "error": str(e),
                }

        results["tasks"][task_name] = task_results

    return results


def format_results_markdown(results: dict) -> str:
    """Format results as markdown tables."""
    lines = [
        "# Output Compression Benchmark Results",
        "",
        f"Model: **{results['meta']['model']}**",
        f"Timestamp: {results['meta']['timestamp']}",
        f"Token estimation: {results['meta']['token_method']}",
        "",
        "## Token Usage by Arm",
        "",
        "| Task | verbose | standard | terse | minimal | caveman-full |",
        "|------|--------:|---------:|------:|--------:|-------------:|",
    ]

    arm_totals = {arm: 0 for arm in ARM_PROMPTS}

    for task_name, task_data in results["tasks"].items():
        row = f"| {task_name} |"
        for arm in ARM_PROMPTS:
            arm_data = task_data["arms"].get(arm, {})
            tokens = arm_data.get("output_tokens", 0)
            arm_totals[arm] += tokens
            row += f" {tokens} |"
        lines.append(row)

    # Totals row
    row = "| **TOTAL** |"
    for arm in ARM_PROMPTS:
        row += f" **{arm_totals[arm]}** |"
    lines.append(row)

    # Savings row
    verbose_total = arm_totals.get("verbose", 1)
    row = "| **Savings vs verbose** |"
    for arm in ARM_PROMPTS:
        if arm == "verbose":
            row += " -- |"
        elif verbose_total > 0:
            savings = (1 - arm_totals[arm] / verbose_total) * 100
            row += f" {savings:.0f}% |"
        else:
            row += " N/A |"
    lines.append(row)

    # Accuracy table
    lines.extend([
        "",
        "## Accuracy by Arm",
        "",
        "| Task | verbose | standard | terse | minimal | caveman-full |",
        "|------|--------:|---------:|------:|--------:|-------------:|",
    ])

    arm_accuracy_sums = {arm: 0.0 for arm in ARM_PROMPTS}
    task_count = len(results["tasks"])

    for task_name, task_data in results["tasks"].items():
        row = f"| {task_name} |"
        for arm in ARM_PROMPTS:
            arm_data = task_data["arms"].get(arm, {})
            acc = arm_data.get("accuracy", 0.0)
            arm_accuracy_sums[arm] += acc
            row += f" {acc:.0%} |"
        lines.append(row)

    # Average accuracy row
    row = "| **Average** |"
    for arm in ARM_PROMPTS:
        avg = arm_accuracy_sums[arm] / task_count if task_count > 0 else 0
        row += f" **{avg:.0%}** |"
    lines.append(row)

    lines.extend([
        "",
        "## Notes",
        "",
        "- Token counts are from Anthropic API usage stats (actual, not estimated)",
        "- Accuracy = count of required_facts substrings found in response / total facts",
        "- Estimated cost: ~$0.50/run with Sonnet (50 API calls)",
        "- This is a local-only benchmark. Not run in CI.",
        "",
    ])

    return "\n".join(lines)


def main():
    parser = argparse.ArgumentParser(
        description="Output compression benchmark: 10 tasks x 5 arms"
    )
    parser.add_argument(
        "--model",
        default="claude-sonnet-4-20250514",
        help="Anthropic model to use (default: claude-sonnet-4-20250514)",
    )
    parser.add_argument(
        "--tasks",
        default=None,
        help="Comma-separated task names or IDs to run (default: all)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Estimate costs without making API calls",
    )
    parser.add_argument(
        "--output-json",
        default=RESULTS_JSON,
        help=f"JSON output path (default: {RESULTS_JSON})",
    )
    parser.add_argument(
        "--output-md",
        default=RESULTS_MD,
        help=f"Markdown output path (default: {RESULTS_MD})",
    )
    args = parser.parse_args()

    # Check API key (unless dry-run)
    if not args.dry_run and not os.environ.get("ANTHROPIC_API_KEY"):
        print("ERROR: ANTHROPIC_API_KEY environment variable required", file=sys.stderr)
        print("Set it with: export ANTHROPIC_API_KEY=sk-ant-...", file=sys.stderr)
        sys.exit(1)

    # Load tasks
    task_filter = args.tasks.split(",") if args.tasks else None
    tasks = load_tasks(task_filter)

    if not tasks:
        print("ERROR: No tasks found. Check evals/tasks/ directory.", file=sys.stderr)
        sys.exit(1)

    print(f"Output Compression Benchmark")
    print(f"Model: {args.model}")
    print(f"Tasks: {len(tasks)}")
    print(f"Arms: {len(ARM_PROMPTS)}")
    print(f"Total API calls: {len(tasks) * len(ARM_PROMPTS)}")
    if args.dry_run:
        print("Mode: DRY RUN (no API calls)")
    else:
        est_cost = len(tasks) * len(ARM_PROMPTS) * 0.01
        print(f"Estimated cost: ~${est_cost:.2f}")
    print()

    results = run_benchmark(tasks, args.model, args.dry_run)

    # Write JSON results
    with open(args.output_json, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
    print(f"\nJSON results: {args.output_json}")

    # Write markdown results
    md = format_results_markdown(results)
    with open(args.output_md, "w", encoding="utf-8") as f:
        f.write(md)
    print(f"Markdown results: {args.output_md}")


if __name__ == "__main__":
    main()
