...
Shared reference for all frontend agents (`fg-320-frontend-polisher`, `frontend-reviewer`, `frontend-a11y-reviewer`). These are actionable design rules distilled from established theory — not textbook abstractions. Agents that CREATE (polisher) use these to guide implementation. Agents that EVALUATE (reviewers) use these as scoring criteria.
