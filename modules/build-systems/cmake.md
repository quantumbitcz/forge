...
Cross-platform meta-build system that generates native build files (Makefiles, Ninja, Visual Studio, Xcode) from `CMakeLists.txt`. De facto standard for C/C++ projects across embedded, games, scientific computing, and systems programming.

- **Use for:** C/C++ projects, polyglot codebases with native components, cross-compilation (ARM, RISC-V, embedded), multi-platform targeting (same CMakeLists.txt produces Makefiles/Ninja/MSVC)
- **Avoid for:** pure managed-language projects (Java, Python, JS, Rust) — each has a superior native build tool; small C interop (use Cargo `build.rs` or Go CGo instead)
- **Key differentiators:** generator (not build tool) — delegates scheduling/parallelism to Ninja/Make/MSVC; target-based modern CMake (3.0+) with dependency propagation; CMake Presets (3.19+) for reproducible invocations; FetchContent (3.11+) for source-level dependency management; CTest + CDash for test aggregation
