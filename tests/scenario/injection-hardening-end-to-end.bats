...
@test "e2e: poisoned Linear ticket — filter wraps, event logged, agent header in place" {
  run python3 - "$TMP/events.jsonl" "$ROOT/agents/fg-020-bug-investigator.md" <<'PY'
import json
import sys
from pathlib import Path

# PYTHONPATH (set in setup) covers ROOT.
from hooks._py import mcp_response_filter as f

events_path = Path(sys.argv[1])
agent_md_path = Path(sys.argv[2])
