...
Before writing any finding, read your dispatch input — it contains a `run_id` field (the current pipeline run identifier) and your agent_id is your name (e.g., `fg-413-frontend-reviewer`). Substitute these into the path: `.forge/runs/{run_id}/findings/{agent_id}.jsonl`.
...
````
.forge/runs/<run_id>/reports/fg-413-frontend-reviewer.md
````
...
write `- (none specific to frontend scope)`.
...
bullet ≤2 sentences. Examples in the frontend domain:

- Component composition is drifting toward render-prop layering for
  features the design system already supports as primitives; reusing the
  existing `Card`/`Stack` reduces visual divergence.
- Several modals trap focus inconsistently; a shared `useFocusTrap` hook
  next refactor would close the a11y gap surfaced this run.
...
change reaches frontend-reviewer), write the report with:
...
## Assessment
**Ready to merge:** Yes
**Reasoning:** No frontend-relevant changes in this diff.
````
