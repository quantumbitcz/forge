...
Before writing any finding, read your dispatch input — it contains a `run_id` field (the current pipeline run identifier) and your agent_id is your name (e.g., `fg-411-security-reviewer`). Substitute these into the path: `.forge/runs/{run_id}/findings/{agent_id}.jsonl`.
...
write `- (none specific to security scope)`.
...
bullet ≤2 sentences. Examples in the security domain:

- Consider centralising the token-validation helper rather than repeating
  the JWT decode at three call sites; a single chokepoint makes future
  algorithm changes auditable.
- Several handlers log request bodies at INFO; route PII-bearing fields
  through a redaction filter before they hit the log stream.
...
change reaches security-reviewer), write the report with:
...
## Assessment
**Ready to merge:** Yes
**Reasoning:** No security-relevant changes in this diff.
````
