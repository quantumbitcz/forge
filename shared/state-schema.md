...
| `abort_reason` | string | No | Reason the pipeline was aborted. Set when the orchestrator auto-aborts. Valid values: `"NO-GO timeout"` (validator returned NO-GO and user did not respond within timeout), `"budget exhausted"` (`total_retries >= total_retries_max`), `"recovery budget exhausted"` (`recovery_budget.total_weight >= max_weight`), `"user abort"` (user chose "Abort" at an escalation prompt), `"convergence regression"` (convergence engine detected REGRESSING state and user aborted). Empty string or absent when not aborted. Present only in terminal state (`complete: true`). |
