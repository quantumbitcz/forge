...
def test_pipeline_readme_see_also() -> None:
    text = (ROOT / "tests" / "evals" / "pipeline" / "README.md").read_text(encoding="utf-8")
    assert "tests/evals/benchmark" in text


def test_observability_mentions_benchmark_spans() -> None:
    text = (ROOT / "shared" / "observability.md").read_text(encoding="utf-8")
...
def test_learnings_readme_lists_benchmark_regression() -> None:
    assert "benchmark.regression" in (ROOT / "shared" / "learnings" / "README.md").read_text(encoding="utf-8")
