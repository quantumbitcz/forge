...
`.forge/security-audit.jsonl` -- one JSON object per line (JSON Lines format). Created on first security event. Survives `/forge-reset`. Rotated when file size exceeds `security.audit_trail.max_file_size_mb` (default 10 MB).
