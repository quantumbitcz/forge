...
- **Project rebuild** (`/forge-admin graph rebuild`): Preserve enrichment properties by default. The rebuild script merges (upserts) rather than deletes+recreates `ProjectFile` nodes, retaining `bug_fix_count` and `last_bug_fix_date`. To clear enrichment, use `/forge-admin graph rebuild --clear-enrichment`.
- **Full reset** (`/forge-admin recover reset`): All data is destroyed, including enrichment.
...
|-----------|---------|-------|
| Initialize | `/forge-admin graph init` | Creates container, loads seed, builds project graph |
| Rebuild project | `/forge-admin graph rebuild` | Deletes `Project*` nodes for the current project (scoped by `project_id`), rebuilds from codebase |
| Incremental update | `/graph-update` via orchestrator | Upserts changed files (scoped by `project_id`/`component`) |
| Query | `/forge-admin graph query` | Execute arbitrary Cypher |
| Status | `/forge-admin graph status` | Container health, node counts, staleness |
| Reset | `/forge-admin recover reset` | Stops container, removes all data |
