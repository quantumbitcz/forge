...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: build-project-graph.sh --project-root /path/to/project" >&2
      exit 1
      ;;
  esac
done
...
while IFS= read -r lang; do
  [[ -z "$lang" ]] && continue
  echo "MERGE (:ProjectLanguage {name: '$(cypher_escape "$lang")'});"
...
for dir in $(printf '%s\n' "${!PACKAGES[@]}" | sort); do
  ...
  echo "CREATE (:ProjectPackage {name: '$(cypher_escape "$pkg_name")', path: '$(cypher_escape "$dir")'});"
done
echo ""
...
emit_dep() {
  ...
  if ! sgrep -qF "$key" "$DEP_TMPFILE"; then
    ...
    echo "CREATE (:ProjectDependency {name: '$(cypher_escape "$name")', version: '$(cypher_escape "$version")', scope: '${scope}', manager: '${manager}'});"
  fi
}
...
for gradle_file in "$PROJECT_ROOT/build.gradle.kts" "$PROJECT_ROOT/build.gradle" "$PROJECT_ROOT/app/build.gradle.kts" "$PROJECT_ROOT/app/build.gradle"; do
  ...
  sgrep -oE '(implementation|api|testImplementation|runtimeOnly|compileOnly)\s*\(\s*"[^"]*"' "$gradle_file" | \
    sed -E 's/^(implementation|api|testImplementation|runtimeOnly|compileOnly)\s*\(\s*"//; s/"$//' | \
...
if [[ -f "$PROJECT_ROOT/requirements.txt" ]]; then
  sgrep -v '^\s*#' "$PROJECT_ROOT/requirements.txt" | sgrep -v '^\s*$' | \
    sed -E 's/\s*#.*//' | sort | while IFS= read -r line; do
...
" 2>/dev/null || true)"
    ...
    if [[ -n "$mapped_module" ]]; then
      echo "MATCH (d:ProjectDependency {name: '$(cypher_escape "$dep_name")'}), (m) WHERE (m:LayerModule OR m:Framework OR m:TestingFramework) AND m.name = '$(cypher_escape "$mapped_module")' CREATE (d)-[:MAPS_TO]->(m);"
    fi
  done < "$DEP_TMPFILE"
fi
echo ""
...
" 2>/dev/null | while IFS=$'\t' read -r key val; do
    ...
    case "$key" in
      language)
        echo "MERGE (pc:ProjectConfig {project: '$(cypher_escape "$PROJECT_ROOT")'}) ON CREATE SET pc.language = '$(cypher_escape "$val")';"
        echo "MATCH (pc:ProjectConfig {project: '$(cypher_escape "$PROJECT_ROOT")'}), (l:Language {name: '$(cypher_escape "$val")'}) CREATE (pc)-[:USES_CONVENTION]->(l);"
        ;;
      framework)
        echo "MATCH (pc:ProjectConfig {project: '$(cypher_escape "$PROJECT_ROOT")'}), (f:Framework {name: '$(cypher_escape "$val")'}) CREATE (pc)-[:USES_CONVENTION]->(f);"
        ;;
      testing)
