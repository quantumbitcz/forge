...
| `convergence.phase_history` | array | Yes | Append-only log of completed phases. Each entry: `{ "phase": "<name>", "iterations": <int>, "outcome": "converged"\|"escalated"\|"restarted", "duration_seconds": <int> }`. Outcome values: `"converged"` (target reached or plateau accepted), `"escalated"` (cap hit, regression, or user escalation), `"restarted"` (safety gate failure triggered correctness restart). Used by retrospective for trend analysis. |
