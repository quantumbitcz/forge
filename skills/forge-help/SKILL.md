...
| New project from scratch | `/forge-bootstrap` | Scaffold project with architecture, CI/CD, tooling |
| Library/framework upgrade | `/forge-migration` | Phased migration plan with rollback |
...
| Failing tests / broken build | `/forge-verify` | Quick build+lint+test check |
| Pipeline stuck or broken | `/forge-diagnose` → `/forge-repair-state` | Read-only diagnostic, then targeted fixes |
...
| Entire codebase health check | `/forge-codebase-health` | Read-only analysis, no fixes |
| Fix all issues iteratively | `/forge-deep-health` | Review → fix → re-review loop until clean |
| Security vulnerabilities | `/forge-security-audit` | Module-appropriate vulnerability scanners |
...
| Validate config before running | `/forge-config-validate` | Check forge.local.md + forge-config.md |
...
| Situation | Skill | What it does |
|-----------|-------|-------------|
| Generate/update docs | `/forge-docs-generate` | README, ADRs, API specs, changelogs |
...
| Explore knowledge graph | `/forge-graph-query` | Run Cypher queries |
