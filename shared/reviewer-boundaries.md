...
| fg-413-frontend-reviewer | A11y (WCAG 2.2 AA), design system, FE performance, responsive, dark mode | Backend caching (→ fg-416) |
| fg-416-performance-reviewer | N+1 queries, missing indexes, connection pools, caching strategy, caching library choice, concurrency | Dependency CVEs (→ fg-417), security implications (→ fg-411) |
| fg-417-dependency-reviewer | Dependency health, CVEs, version conflicts, license compliance, outdated/unmaintained packages | Runtime performance (→ fg-416), architecture fit (→ fg-412) |
...
- CVE detection, outdated packages, unmaintained libraries, license compliance → fg-417-dependency-reviewer
