...
1. **Run discovery script:**
   ...
   This scans in order: in-project references (docker-compose.yml, .env files, CI workflow references), sibling directories (same parent, compatible stack markers), IDE project directories (IdeaProjects, WebstormProjects, PycharmProjects, GolandProjects, CLionProjects, RiderProjects, AndroidStudioProjects, Projects, Developer, workspace, repos), and GitHub org repos (if `gh` CLI is authenticated).
