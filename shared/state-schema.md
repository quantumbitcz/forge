...
+-- playbook-analytics.json             # Playbook usage analytics (survives /forge-reset)
...
|-------|------|----------|-------------|
| `version` | string | Yes | Schema version string (`"1.5.0"`). Enables schema compatibility checks — the recovery engine checks this before parsing. If the version is missing, the state file is reinitialized. If the version is older, sequential migrations are applied (see [Version Migration](#version-migration)). v1.5.0 adds `_seq` (write versioning), `previous_state` (for user_continue recovery), `convergence.diminishing_count`, `convergence.unfixable_info_count`. v1.4.0 adds optional `evidence` section for pre-ship verification tracking. v1.3.0 adds optional `decomposition` section and `visual_companion` integration. v1.2.0 adds the optional `graph` section for graph update state tracking. v1.1.0 added optional tracking fields. |
...
- **Survives `/forge-reset`** -- only manual `rm -rf .forge/` removes it.
