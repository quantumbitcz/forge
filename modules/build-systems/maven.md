...
Declarative JVM build tool using a fixed lifecycle, standard directory layout, and POM XML descriptors. Most widely deployed in the Java ecosystem; its strength is predictability — every Maven project follows the same conventions. BOM pattern is the de facto standard for version alignment.

- **Use for:** enterprise Java valuing ecosystem compatibility, Maven Central publishing, orgs mandating declarative XML builds, teams prioritizing familiarity over customization
- **Avoid for:** non-JVM projects, Android (Gradle only), large multi-module builds (15+ modules — no config cache or task avoidance), builds needing conditional logic or dynamic task graphs
- **vs Gradle:** fixed lifecycle vs task DAG, pure XML (no procedural logic), "nearest wins" dependency mediation vs "highest version wins", no build cache equivalent
