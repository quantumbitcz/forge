...
@test "session-start: emits caveman instructions when caveman-mode file exists" {
  ...
  run_hook_in "$project_dir"
  assert_success
  [[ "$output" == *"Caveman mode active"* ]] || fail "Expected caveman full message, got: $output"
}
...
@test "session-start: emits lite caveman message" {
  ...
  run_hook_in "$project_dir"
  assert_success
  [[ "$output" == *"Caveman lite active"* ]] || fail "Expected caveman lite message, got: $output"
}
...
@test "session-start: emits ultra caveman message" {
  ...
  run_hook_in "$project_dir"
  assert_success
  [[ "$output" == *"CAVEMAN ULTRA"* ]] || fail "Expected caveman ultra message, got: $output"
}
