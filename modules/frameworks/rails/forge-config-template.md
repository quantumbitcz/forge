...
| fg-410-code-reviewer | true | secondary | RuboCop/Rails-style violations — findings merged into quality-gate |
| fg-411-security-reviewer | true | elevated (1.3) | Strong params, mass assignment, raw SQL, CSRF — Rails foot-guns warrant elevated scrutiny |
| fg-416-performance-reviewer | true | elevated (1.2) | N+1 from missing `includes`, `update_all` without scope, `deliver_now` in request paths |
| fg-419-infra-deploy-reviewer | conditional | secondary | Build, CI/CD, container & orchestration — dispatched when `build_system`, `ci`, `container`, or `orchestrator` is configured |
...
# Rails: prefer mutant-rspec when enabled. Coverage target: services/, queries/, forms/
mutation_testing:
  enabled: false
...
# Visual verification (v1.18+) — useful for Hotwire UIs
visual_verification:
  enabled: false
  dev_server_url: ""
...
# LSP integration (v1.18+) — Solargraph or Ruby LSP
lsp:
  enabled: true
  languages: []
...
implementer:
  inner_loop:
    ...
    lint_cmd: "bundle exec rubocop --parallel"
    ...
    code_graph:
      ...
      exclude_patterns: [node_modules, .git, vendor, build, dist, tmp, log, public/assets, public/packs]
...
property_testing:
  enabled: false                 # Opt-in (rspec + rantly or hypothesis-rb)
...
performance_tracking:
  enabled: false                 # Opt-in (rails-mini-profiler or benchmark-ips)
  thresholds:
    build_time_pct: 20
...
# Accessibility Automation (v2.0+) — Hotwire/ERB pages benefit from this
accessibility:
  dynamic_checks: true
  cross_browser: false           # Opt-in (adds latency)

# i18n Validation (v2.0+) — Rails has first-class I18n; recommended on
i18n:
...
# Rails 8 ships with Kamal 2; canary/rolling supported via Kamal hooks
deployment:
...
contract_testing:
  provider: auto                 # pact-ruby supported
  can_i_deploy: true
...
# Feature Flags (v2.0+) — flipper / unleash-ruby supported
feature_flags:
  enabled: false                 # Opt-in
  provider: auto
