...
def test_run_id_never_appears_in_span_names(monkeypatch):
    ...
    for span in exporter.get_finished_spans():
        ...
        assert run_id not in span.name, (
            f"run_id leaked into span name: {span.name}"
        )
    # run_id must still be present as an attribute on the pipeline span.
    pipeline = next(
        s for s in exporter.get_finished_spans() if s.name == "pipeline"
    )
...
def test_span_names_are_enumerable():
    ...
    for prefix in allowed_prefixes:
        ...
        assert prefix in src, (
            f"expected span-name prefix {prefix!r} in otel.py source"
        )
...
def test_no_unbounded_attribute_value_in_span_names(monkeypatch):
    ...
    with otel.pipeline_span(run_id="r-x", mode="standard"):
        with otel.tool_span(name="Read", call_id=call_id):
            pass
    otel.shutdown()
    for span in exporter.get_finished_spans():
        assert call_id not in span.name, (
            f"tool call_id leaked into span name: {span.name}"
        )
    tool = next(
