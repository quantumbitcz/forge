...
| `mode` | string | Yes | Pipeline execution mode detected from requirement prefix. Valid values: `"standard"` (default), `"migration"` (requirement starts with `migrate:` or `migration:`), `"bootstrap"` (requirement starts with `bootstrap:`), `"bugfix"` (requirement starts with `bugfix:` or invoked via `/forge-fix`). Determines which planner agent is dispatched at Stage 2. |
