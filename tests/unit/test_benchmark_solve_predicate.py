...
from tests.evals.benchmark.scoring import SolveInputs, solved


@pytest.mark.parametrize(
    "verdict,ac_pct,criticals,expected",
    [
        ("SHIP", 1.0, 0, True),
        ("SHIP", 0.9, 0, True),
        ("CONCERNS", 0.9, 0, True),
...
def test_unverifiable_counts_against_ac_pct() -> None:
    ...
    assert compute_partial_ac_pct(
        {"A": "PASS", "B": "PASS", "C": "PASS", "D": "UNVERIFIABLE"}
    ) == pytest.approx(0.75)
